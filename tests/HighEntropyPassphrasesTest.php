...
require_once('src/HighEntropyPassphrases.php');

class HighEntropyPassphrasesTest extends TestCase {
    public function testCase1()
    {
        $this->assertTrue(HighEntropyPassphrases::validate('aa bb cc dd ee'));
    }
    public function testCase2()
    {
        $this->assertFalse(HighEntropyPassphrases::validate('aa bb cc dd aa'));
    }
    public function testCase3()
    {
        $this->assertTrue(HighEntropyPassphrases::validate('aa bb cc dd aaa'));
    }
...
EOF;
    ...
    {
        $numValidPassphrase = 0;
        foreach(explode("\n", $this->getFinalData()) as $line) {
            if (HighEntropyPassphrases::validate($line)) ++$numValidPassphrase;
        }
        $this->assertEquals(451, $numValidPassphrase);
    }
    public function testFinalTwo()
    {
        $numValidPassphrase = 0;
        foreach(explode("\n", $this->getFinalData()) as $line) {
            if (HighEntropyPassphrases::validateNoAnagrams($line)) ++$numValidPassphrase;
        }
        $this->assertEquals(223, $numValidPassphrase);
    }
}
